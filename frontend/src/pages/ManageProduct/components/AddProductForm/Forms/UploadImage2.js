...
export default function UploadImage1({ image2, imageAdded2 }) {
  const classes = useStyles();
  const [avarta2, setAvarta2] = useState(IMAGE_PLACEHOLDER);
  ...
  const handleChange = (e) => {
    ...
    if (imageAdded2.image2.length > 0) {
      setAvarta2(`${URL_IMAGE}${imageAdded2.image2}`);
      image2(imageAdded2.image2);
      return;
    }
    setAvarta2(IMAGE_PLACEHOLDER);
    image2([]);
  };
    ...
    <Stack direction="column" className={classes.img}>
      ...
      <Box className={classes.imageBtn}>
        <Box className={classes.btn}>
          ...
          <Box>
            <Button
              disabled={avarta2 === IMAGE_PLACEHOLDER}
              variant="contained"
              color="error"
              size="small"
