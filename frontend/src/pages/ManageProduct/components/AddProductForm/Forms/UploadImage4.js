...
export default function UploadImage1({ image4, imageAdded4 }) {
  const classes = useStyles();
  const [avarta4, setAvarta4] = useState(IMAGE_PLACEHOLDER);
  useEffect(() => {
    if (imageAdded4.image4.length > 0) {
      setAvarta4(`${URL_IMAGE}${imageAdded4.image4}`);
    }
  }, []);
  const handleChange = (e) => {
    ...
    if (e.target.files.length > 0) {
      ...
      setAvarta4(imageAdded4.image4);
      image4(imageAdded4.image4);
    }
  };

  const handleDeleteImage = () => {
    setAvarta4(IMAGE_PLACEHOLDER);
    image4([]);
  };
    ...
    <Stack direction="column" className={classes.img}>
      ...
      <Box className={classes.imageBtn}>
        <Box className={classes.btn}>
          ...
          <Box>
            <Button
              disabled={avarta4 === IMAGE_PLACEHOLDER}
              variant="contained"
              color="error"
              size="small"
